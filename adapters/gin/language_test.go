...
func TestLanguageFromQueryParam(t *testing.T) {
	router := gin.New()
	router.Use(apikitgin.Language(apikitgin.LanguageConfig{
		Supported: []string{"en", "ja", "ko"},
		Default:   "en",
	}))
	router.GET("/test", func(c *gin.Context) {
		lang := apikitgin.GetLanguage(c)
...
func TestGetLanguageFromGinContext(t *testing.T) {
	router := gin.New()
	router.Use(apikitgin.Language(apikitgin.LanguageConfig{Supported: []string{"en", "ja"}}))
	router.GET("/x", func(c *gin.Context) { c.String(http.StatusOK, apikitgin.GetLanguage(c)) })
...
func TestGetLanguageWithoutMiddleware(t *testing.T) {
	...
	if got := apikitgin.GetLanguage(c); got != "en" {
...
func TestLanguageFromCookie(t *testing.T) {
	router := gin.New()
	router.Use(apikitgin.Language(apikitgin.LanguageConfig{Supported: []string{"en", "ja", "ko"}}))
	router.GET("/x", func(c *gin.Context) { c.String(http.StatusOK, apikitgin.GetLanguage(c)) })
...
func TestLanguagePrecedence(t *testing.T) {
	...
	router := gin.New()
	router.Use(apikitgin.Language(apikitgin.LanguageConfig{Supported: []string{"en", "ja", "ko", "zh"}}))
	router.GET("/:lang/x", func(c *gin.Context) { c.String(http.StatusOK, apikitgin.GetLanguage(c)) })
...
func TestExtractLanguageFromPath(t *testing.T) {
	...
	for path, want := range cases {
		if got := apikitgin.ExtractLanguageFromPath(path); got != want {
...
func TestBuildSupportedMap(t *testing.T) {
	m := apikitgin.BuildSupportedMap([]string{"EN", "Ja"})
	if _, ok := m["en"]; !ok {
...
func TestParseAcceptLanguageQValues(t *testing.T) {
	supported := apikitgin.BuildSupportedMap([]string{"en", "ja", "ko"})
	if got := apikitgin.ParseAcceptLanguage("ko;q=0.2,ja;q=0.9,en;q=0.5", supported); got != "ja" {
		t.Errorf("got %q, want the highest q-value", got)
	}
	if got := apikitgin.ParseAcceptLanguage("de,fr", supported); got != "" {
		t.Errorf("got %q, want empty when nothing is supported", got)
	}
	if got := apikitgin.ParseAcceptLanguage("ja-JP", supported); got != "ja" {
...
func TestSetLanguageCookie(t *testing.T) {
	...
	apikitgin.SetLanguageCookie(c, "ja")
