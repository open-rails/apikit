...
func TestMetadataRefusesInternalDetail(t *testing.T) {
	apikit.RegisterMetadataKey("provider_response", "detail", "stack", "token", "operator_email", "callback_url", "note")
	...
	for k, v := range cases {
		t.Run(k, func(t *testing.T) {
			got := apikit.SanitizeMetadata(map[string]any{k: v})
			if got != nil {
				t.Fatalf("%s survived sanitization: %#v", k, got)
			}
			if why := apikit.MetadataRejections(map[string]any{k: v}); why[k] == "" {
...
func TestMetadataAllowlist(t *testing.T) {
	...
	got := apikit.SanitizeMetadata(in)
	...
	apikit.RegisterMetadataKey("undocumented")
	if got := apikit.SanitizeMetadata(in); got["undocumented"] != "anything" {
...
func TestMetadataReservedKeysCannotBeRegistered(t *testing.T) {
	...
	apikit.RegisterMetadataKey(reserved...)
	allowed := strings.Join(apikit.AllowedMetadataKeys(), ",")
	for _, k := range reserved {
		if strings.Contains(","+allowed+",", ","+k+",") {
			t.Errorf("reserved key %q was allowlisted", k)
		}
		if got := apikit.SanitizeMetadata(map[string]any{k: "x"}); got != nil {
...
func TestMetadataBounds(t *testing.T) {
	apikit.RegisterMetadataKey("note")
	t.Run("nested values are refused", func(t *testing.T) {
		apikit.RegisterMetadataKey("nested")
		if got := apikit.SanitizeMetadata(map[string]any{"nested": map[string]any{"inner": 1}}); got != nil {
			t.Errorf("a nested map reached the wire: %#v", got)
		}
	})
	t.Run("long strings are refused", func(t *testing.T) {
		if got := apikit.SanitizeMetadata(map[string]any{"note": strings.Repeat("a", 500)}); got != nil {
			t.Errorf("an oversized string reached the wire: %#v", got)
		}
	})
	t.Run("the whole map is bounded", func(t *testing.T) {
		in := map[string]any{}
		for _, k := range apikit.AllowedMetadataKeys() {
...
func TestSanitizationCannotBeBypassed(t *testing.T) {
	apikit.RegisterMetadataKey("detail")
	leak := map[string]any{"detail": `pq: violates unique constraint "x"`, "retry_after": 5}

	env := apikit.NewEnvelope(apikit.ErrorObject{Type: apikit.TypeAPI, Message: "x", Metadata: leak})
	if _, ok := env.Error.Metadata["detail"]; ok {
		t.Error("NewEnvelope let a leak through")
	}
	e := apikit.E(http.StatusConflict, apikit.CodeResourceConflict, "x").WithMetadata(leak)
	if _, ok := e.Metadata["detail"]; ok {
		t.Error("WithMetadata let a leak through")
	}
	_, viaErr := apikit.EnvelopeFor(e)
